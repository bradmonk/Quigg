...
clc; close all; clear;
cd(fileparts(which('TRUCKSTUDY_RAWIMPORT.m')));
...
YE = repmat(YEAR',size(ESALDAT,2),1);
...
clearvars -except TAB EBWB ESAL STATA SYEAR ORIG REHAB P S M L LANES...
 RDATE RDATES HWYSTART HWYSTOP RYEAR RDATE YEAR ESALDAT SAL DIST
...
REDATES = REHAB.DMYRehab_Date;
% ESALDAT

HIGHWAY = repmat(ORIG.SHEET,size(ORIG.Primary_Route));

clear IDOT DOT
IDOT.HIGHWAY     = HIGHWAY;
IDOT.ZONE        = (1:size(HIGHWAY,1))';
IDOT.COUNTY      = ORIG.Primary_County;
IDOT.MP_START    = HWYSTART;
IDOT.MP_END      = HWYSTOP;
...
IDOT.ODATE       = ORIG.DMYCompletion_Date;
IDOT.RYEAR       = RYEAR;
IDOT.RDATE       = RDATE;
...
% IDOT.ESAL      = ESALDAT';
IDOT.EYR1        = SAL.firstyear(:,1)


DOT = struct2table(IDOT);


if any(IDOT.OYEAR > SAL.firstyear(:,1))
...
ESALS      = array2table(EDATA);

EYEAR      = EFULL(2:end,1);
YYYY       = char(string(EFULL(2:end,1)));
YR         = repmat('ESAL',size(YYYY,1),1);
YRYYYY     = cellstr([YR YYYY]);

ESALS.Properties.VariableNames = YRYYYY;
...
clearvars -except TAB EBWB ESAL STATA SYEAR ORIG REHAB P S M L LANES...
 RDATE RDATES HWYSTART HWYSTOP RYEAR RDATE YEAR ESALDAT...
 IDOT ESALS EYEAR TC
